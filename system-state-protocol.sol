...
contract StateToken is ERC20, Ownable(msg.sender) {
    ...
    // Address to send X1 rebate funds after day 36.9
    address constant x1RebateWallet =
        0x7B16e35A4658dbc905e830131f5Fb758F6e46738;

    // Boolean to track if day 36.9 has passed
    bool public day36_9Passed;

    // Modifier to restrict access after day 36.9
    modifier onlyBeforeDay36_9() {
        require(
            !day36_9Passed,
            "X1 rebate can only be claimed before day 36.9"
        );
        _;
    }

    // Function to check if day 36.9 has passed
    function checkDay36_9Passed() internal {
        // Logic to determine if day 36.9 has passed
        // You need to implement the logic to determine if day 36.9 has passed
        // This could involve checking the timestamp or another mechanism
        // Set day36_9Passed to true if day 36.9 has passed
    }
    ...
    event ClaimTarget(
        ...
        uint256 thisTargetAmount,
        uint256 RefundReward,
        uint256 X1AllocationReward,
        uint256 AdminReward
    );
    ...
    function tokenCalculate(
        uint256 _userUsdValue
    ) public view returns (uint256) {
        return
            ((_userUsdValue).mul(FIXED_POINT).div(StateTokenPrice))
                .mul(10 ** decimals())
                .div(FIXED_POINT);
    }
    ...
    function getUsdcSpendOnInscription(
        address _address
    ) public view returns (uint256) {
    ...
    // Function to inscribe and receive tokens
    function inscribe() public payable {
        ...
        _mint(msg.sender, StateTokenTransfer);
        ...
        require(success, "Admin reward transaction has failed.");
    ...
    function claimTargetsByBackend() public onlyBackend onlyBeforeDay36_9 {
        checkDay36_9Passed();
            ...
            for (uint256 j = 0; j < targetMapping[thisUser].length; j++) {
                ...
                if (!target.isClosed && price() >= target.ratioPriceTarget) {
                    ...
                    for (uint256 k = 0; k < Holders.length; k++) {
                        ...
                        uint256 distributeEachTargetPercentage = (balanceOf(
                            user
                        ) *
                            FIXED_POINT *
                            10000) / totalSelledTokens;
                        uint256 ThisTargetAmount = (target.TargetAmount *
                        ...
                        uint256 refundReward = ThisTargetAmount.mul(382).div(
                            1000
                        ); // ● Refund reward - 38.2%
        ...
        if (day36_9Passed) {
            (bool success, ) = payable(x1RebateWallet).call{value: surplusFund}(
                ""
            );
            require(
                success,
                "Transfer of surplus fund to X1 rebate wallet failed."
            );
        }
    ...
    // Function to withdraw refund rewards
    function withdrawRefundReward() public {
        ...
        require(
            refundAmount > 0,
            "You do not have enough balance in the refund reward bucket."
        );

        (bool success, ) = payable(sender).call{value: refundAmount}("");
        require(success, "An error occurred in the refund reward transaction.");

        refundRewardClaimableBucket[sender] = 0;
    }

    // Function to withdraw X1 allocation reward
    function withdrawX1allocationReward() public {
        address sender = msg.sender;
        uint256 x1allocationAmount = X1allocationClaimableBucket[sender];
        require(
            x1allocationAmount > 0,
            "You do not have enough balance in X1 allocation bucket."
        );

        (bool success, ) = payable(sender).call{value: x1allocationAmount}("");
        require(success, "An error occurred in X1 allocation transaction.");
